...
class ProfileActivity : AppCompatActivity() {
    ...
    // P1 2024: Determine the Patient's BP Stage
    ...
    private fun updateUIWithPatientData(document: DocumentSnapshot, patientID: String) {
        ...
        binding.legalNameTV.text =
            AESEncryption().decrypt(document.getString("legalName").toString())
        ...
        binding.weightTV.text =
            "${AESEncryption().decrypt(document.getString("weight").toString())} kg"

        binding.heightTV.text =
            "${AESEncryption().decrypt(document.getString("height").toString())} cm"
